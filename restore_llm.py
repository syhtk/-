...
# Fix the mess at the end of the file. I will just rewrite the bottom part.

fixed_bottom = '''    def retrieve_knowledge_base(self, category: str, defects: List[str]) -> str:
        \"\"\"
    ...
    def generate(self, analysis: AnalysisResult, profile: UserProfile, kg: Dict) -> tuple:
        \"\"\"
        ...
        if not response:
            response = self.simulate_llm_generation(prompt, profile, analysis, category_name)
            
        return prompt, response + "\\n\\n`json\\n{\\"action\\": \\"issue_coupon\\", \\"amount\\": 50, \\"reason\\": \\"客服安抚补偿\\"}\\n`"
    ...
    def call_real_llm(self, prompt: str) -> str:
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "你是一位专业的电商金牌客服。你还需要执行 Tool Calling。"},
...
# we will slice content before the first def retrieve_knowledge_base
idx = content.find('    def retrieve_knowledge_base')
if idx != -1:
    content = content[:idx] + fixed_bottom
else:
    # Just in case
    idx = content.find('    def generate')
    content = content[:idx] + fixed_bottom

open('review_response_system.py', 'w', encoding='utf-8').write(content)
